...
// Escape `text` so it can be embedded as a C string literal between double
// quotes. Used to encode raw doxygen comments into PASTA_METHOD_METADATA
// arguments without breaking the X-macro expansion.
static std::string EscapeForCStringLiteral(llvm::StringRef text) {
  std::string out;
...
// Pull the raw doxygen comment text for `decl` if any. Returns "" when the
// declaration has no associated doxygen.
static std::string DoxygenFor(const clang::ASTContext *ctx,
                              const clang::Decl *decl) {
  const clang::RawComment *rc = ctx->getRawCommentForDeclNoCache(decl);
  if (!rc) {
    return std::string{};
  }
  return rc->getRawText(ctx->getSourceManager()).str();
}
...
MacroGenerator::~MacroGenerator(void) {
  ...
  // Sibling artifact: per-method metadata consumed by the metadata-curation
  // skill. One PASTA_METHOD_METADATA line is emitted for every method that
  // also lands in Generated.h's PASTA_INSTANCE_METHOD_* / PASTA_OVERRIDE_*.
  std::error_code mm_ec;
  llvm::raw_fd_ostream mm_os(kClangMethodMetadataHeader, mm_ec);
  if (mm_ec) {
    std::cerr << "Error opening method metadata header: " << mm_ec.message();
    return;
  }
  mm_os << "// This file is auto-generated by bootstrap-macros.\n"
        << "// One PASTA_METHOD_METADATA(...) per wrapped method, parallel to\n"
        << "// the PASTA_INSTANCE_METHOD_* / PASTA_OVERRIDE_METHOD_* lines in\n"
        << "// Generated.h. See bin/BootstrapTypes/GeneratedSchema.md.\n\n"
    ...
    for (const auto &[method_name, methods] : decl_methods) {
      ...
      if (methods.size() != 1u) {
        ...
        const bool has_deprecated =
            ...
            method->hasAttr<clang::UnavailableAttr>();
        const bool has_hidden_visibility =
