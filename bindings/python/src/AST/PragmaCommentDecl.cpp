...
void RegisterPragmaCommentDecl(py::module_ &m) {
  py::class_<PragmaCommentDecl, Decl>(m, "PragmaCommentDecl")
    ...
    .def("__eq__", [](const PragmaCommentDecl& a, const PragmaCommentDecl& b) { return a.RawDecl() == b.RawDecl(); })
    .def_property_readonly("Argument", &PragmaCommentDecl::Argument)
    .def_property_readonly("CommentKind", &PragmaCommentDecl::CommentKind);
}
} // namespace pasta
