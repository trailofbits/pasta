...
class PatchedMacroTracker : public clang::PPCallbacks {
 ...
 public:
  ...
  void CloseUnclosedExpansion(const clang::Token &tok) {
    ...
    if (prev_role != TokenRole::kIntermediateMacroExpansionToken &&
...
//    MacroToken rc = MacroNode(ast->shared_from_this(), node).RightCorner();
//    size_t index = std::get<size_t>(*reinterpret_cast<const Node *>(rc.impl));
//    const TokenImpl &tok = ast->tokens[index];
//    clang::SourceLocation tok_loc = tok.Location();
//    if (tok_loc.isFileID()) {
//      ast->AppendMarker(tok_loc.getLocWithOffset(tok.data_len),
//                        TokenRole::kEndOfMacroExpansionMarker);
//    } else {
//      ast->AppendMarker(clang::SourceLocation(),
//                        TokenRole::kEndOfMacroExpansionMarker);
//    }
  }
...
//  void DoChangeTokenState(const clang::Token &tok) {
//    if (ast->tokens.empty()) {
//      return;
//    }
//    const TokenImpl &prev_tok = ast->tokens.back();
...
//  }
  ...
  void DoToken(const clang::Token &tok, uintptr_t) {
    ...
    if (::pasta::TryReadRawToken(sm, lo, tok, &tok_data)) {
      ...
      // In handling `_Pragma`, we use the `_Pragma` location as the `Hash`
      // location for the expanded `#pragma`.
    ...
    if (last_directive != nullptr) {
      ...
      for (Node &node : last_directive->nodes) {
        if (std::holds_alternative<size_t>(node)) {
          TokenImpl &tok = ast->tokens[std::get<size_t>(node)];
      ...
      assert(std::holds_alternative<size_t>(last_directive->macro_name));
    }
